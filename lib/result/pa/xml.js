...
export function parse_result(str) {
  return xml(str).then(doc => {
    ...
    const leave_votes = parseInt(leave.votes);
    const remain_votes = parseInt(remain.votes);
    const turnout = parseInt(proposition.$.turnout);
    return {
      ...
      remain_percentage_share: percent(remain_votes, turnout),
      leave_votes: leave_votes,
      leave_percentage_share: percent(leave_votes, turnout)
    };
  });
}
